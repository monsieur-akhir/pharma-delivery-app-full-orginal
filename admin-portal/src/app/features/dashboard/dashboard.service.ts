...
export class DashboardService {
  ...
  getDashboardStats(): Observable<any> {
    return this.http.get<any>(`${this.apiUrl}/stats`).pipe(
      catchError(error => {
        console.error('Error fetching dashboard stats', error);
        // Return default structure for dashboard stats
        return of({
   ...
   */
  getAuditLogs(page: number = 0, pageSize: number = 10): Observable<{data: any[], total: number}> {
  ...
  getRecentOrders(): Observable<any[]> {
    return this.http.get<any[]>(`${this.apiUrl}/recent-orders`).pipe(
      catchError(() => of([]))
    );
  }

  /**
   * Get all chart data for a specific period
   */
  getChartData(period: 'daily' | 'weekly' | 'monthly' | 'yearly'): Observable<{
    orders: ChartData,
    revenue: ChartData,
    users: ChartData
  }> {
    return forkJoin({
      orders: this.getOrdersChartData(),
      revenue: this.getRevenueChartData(period),
      users: this.getUsersDistributionData()
    }).pipe(
      catchError(error => {
